...
export default function Dashboard() {
    ...
    const { data: stats, isLoading: statsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/account/stats?mode=${viewMode}`, {
    ...
    const { data: history, isLoading: historyLoading } = useQuery({
        ...
        queryFn: async () => {
            if (!token) return [];
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/account/history?mode=${viewMode}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!res.ok) return [];
